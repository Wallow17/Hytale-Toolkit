 ...
 *
 * Usage:
 *   EMBEDDING_PROVIDER=voyage VOYAGE_API_KEY=xxx npm run ingest-client [client-data-path]
 *   EMBEDDING_PROVIDER=ollama npm run ingest-client [client-data-path]
 *   npm run ingest-client [client-data-path] --full  # Force full re-index
 */

import { parseClientUIDirectory, getClientUIStats, type ClientUIChunk } from "./client-ui-parser.js";
import { embedClientUIChunks, getModelName, type EmbeddedClientUIChunk, type IngestEmbeddingConfig } from "./embedder.js";
import {
  createClientUITable,
  tableExists,
  getClientUIFileHashes,
  deleteClientUIByFilePaths,
  addClientUIChunks,
} from "./db.js";
...
async function main() {
  ...
  const clientDataPath = nonFlagArgs[0] || DEFAULT_CLIENT_DATA;
  ...
  let chunksToEmbed: ClientUIChunk[] = chunks;
  let filesToDelete: string[] = [];
  let isIncremental = false;

  if (hasExistingTable) {
    console.log("Step 2: Checking for changes (incremental mode)...");
    const existingHashes = await getClientUIFileHashes(dbPath);
  ...
  if (chunksToEmbed.length > 0) {
    ...
    console.log(
      `Step ${embedStepNum}: Embedding ${isIncremental ? "changed " : ""}UI files with ${embeddingConfig.provider} (${modelName})...`
    );
    const startEmbed = Date.now();

    try {
      embeddedChunks = await embedClientUIChunks(chunksToEmbed, embeddingConfig, (current, total) => {
  ...
  try {
    if (isIncremental) {
      ...
      if (filesToDelete.length > 0) {
        const deleted = await deleteClientUIByFilePaths(dbPath, filesToDelete);
        console.log(`  Deleted ${deleted} entries from ${filesToDelete.length} files`);
      }
      if (embeddedChunks.length > 0) {
        await addClientUIChunks(dbPath, embeddedChunks);
