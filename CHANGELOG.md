...
### Fixed
- Folder picker hanging on Linux/Wayland desktops (e.g. CachyOS) — now uses zenity or kdialog instead of tkinter on Linux
...
### Fixed
- Git clone errors during setup now show the actual error message instead of generic "Download failed (code 128)"
- Fixed git stderr capture losing error messages due to overwrite by progress output

## [1.0.16] - 2025-01-31

### Fixed
- PATH detection now supports Windows Store Python and pythoncore installations
- Fixes "Could not find Python Scripts directory" for Python installed at `%LOCALAPPDATA%\Python`

## [1.0.15] - 2025-01-31

### Fixed
- Eliminated console window flash when navigating to IDE Integration page on Windows
- CLI tools page no longer freezes briefly when checking installation status
- Removed `shell=True` from subprocess calls to prevent console windows appearing
